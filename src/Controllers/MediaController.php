...
class MediaController extends Controller
{
    ...
    public function getFile(Request $request, $file = false)
    {
        ...
        $mimetype = strtolower(\File::mimeType($path));
        if(str_start($mimetype, 'image')){
            return $this->imageResponse($request, $path, $file);
        }
     ...
     */
    private function imageResponse(Request $request, string $path, string $file)
    {
        $cacheName = $file.$request->get('w','').$request->get('h','').$request->get('q','');
        ...
        $image = Cache::remember($cacheName, $keepAlive, function () use($request, $path) {
            ...
            $driver     = $request->get('d', config('laravel-filemanager.media.driver', 'resize'));
